...
| Term                             | Meaning                                                                                                                                                      |
| -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Storage Engine  | MySQL 的儲存引擎，負責資料儲存與讀取方式，如 InnoDB（預設，支援事務）與 MyISAM（讀取快速但無事務）。|
| Index | 索引結構加速查詢，如 B+樹索引。InnoDB 主鍵為叢集索引，資料行與索引共存。|
| Undo Log| 事務回滾與 MVCC 使用的日誌，記錄修改前舊資料版本，支持一致性讀取。|
| Redo Log| WAL 日誌確保資料持久性，記錄物理修改，用於崩潰恢復。|
| Query Optimizer | 查詢最佳化器，分析 SQL 選擇最低成本執行計畫（如索引使用順序）。|
| Buffer Pool | InnoDB 記憶體快取區，存放資料頁與索引頁，減少磁碟 I/O。|
| Doublewrite Buffer | 先將髒頁寫入雙重緩衝區再寫資料檔案，防止頁寫入中斷損壞。|
| Change Buffer | 緩衝二級索引變更操作，合併後批量應用，優化插入效能。|
| MVCC | 多版本並存控制，透過 Undo Log 實現讀寫不互斥，快照讀取。|
| Lock | 鎖機制確保事務隔離，包括行鎖、表鎖、間隙鎖防止幻讀。|
