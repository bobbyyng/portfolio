...
<BilingualHeading level={2} en="What it does" zh="功能一覽" />

<Table
  caption={{ en: "Core capabilities", zh: "核心能力" }}
  columns={[
    { key: "area", header: { en: "Area", zh: "範疇" } },
    { key: "capability", header: { en: "Capability", zh: "能力" } },
  ]}
  data={[
    {
      area: { en: "Core ledger", zh: "核心記帳" },
      capability: {
        en: "Income / expense, categories, multi-account, multi-currency (default HKD).",
        zh: "收入／支出、分類、多帳戶、多貨幣（預設 HKD）。",
      },
    },
    {
      area: { en: "Subscriptions", zh: "訂閱" },
      capability: {
        en: "Recurring charges; a transaction is created only on the billing day — future periods are not pre-generated.",
        zh: "循環扣款；只在扣款當日產生交易，不會預先生成未來期數。",
      },
    },
    {
      area: { en: "Schedules", zh: "排程交易" },
      capability: {
        en: "Clone a transaction into daily / weekday auto-entries via cron.",
        zh: "由一筆交易複製成每日／指定星期，由排程自動記帳。",
      },
    },
    {
      area: { en: "Budgets & savings", zh: "預算／儲蓄" },
      capability: {
        en: "Parent-category budgets and savings goals.",
        zh: "按分類父層設定預算與儲蓄目標。",
      },
    },
    {
      area: { en: "Sharing", zh: "帳戶共享" },
      capability: {
        en: "Account owners can share write access with other users; invite tokens gate registration.",
        zh: "帳戶主人可分享寫入權；邀請 token 控制註冊。",
      },
    },
    {
      area: { en: "AI & OCR", zh: "AI／收據" },
      capability: {
        en: "Chat agent with read-only tools; separate receipt parse API via Grok vision.",
        zh: "Chat agent（以讀取為主）；收據解析走獨立 API。",
      },
    },
    {
      area: { en: "PWA & push", zh: "PWA／推送" },
      capability: {
        en: "Add to home screen; daily reminder and pre-billing Web Push (VAPID).",
        zh: "可加至主畫面；每日記帳提醒與扣款前提醒。",
      },
    },
  ]}
/>

<BilingualHeading level={2} en="Three-step bookkeeping" zh="三步記帳" />
...
<BilingualHeading level={2} en="Architecture at a glance" zh="架構速覽" />
...
<BilingualHeading level={3} en="Tech stack" zh="技術棧" />

<Table
  caption={{ en: "Layers and choices", zh: "分層與選擇" }}
  columns={[
    { key: "layer", header: { en: "Layer", zh: "層" } },
    { key: "choice", header: { en: "Choice", zh: "技術" } },
  ]}
  data={[
    {
      layer: { en: "App", zh: "應用" },
    ...
    },
    {
      layer: { en: "API", zh: "介面" },
      choice:
        "Route Handlers under app/api — auth, ledger, reports, chat, OCR, notifications",
    },
    {
      layer: { en: "Data", zh: "資料" },
    ...
    },
    {
      layer: { en: "AI", zh: "智能" },
      choice:
        "Vercel AI SDK ToolLoopAgent; chat tools scoped to the signed-in user; receipt OCR via xAI Grok",
    },
    {
      layer: { en: "Deploy", zh: "部署" },
...
<BilingualHeading level={2} en="Business rules that matter" zh="幾條重要業務規則" />
...
<BilingualHeading level={2} en="AI assistant shape" zh="AI 助手如何組成" />
...
<BilingualHeading level={2} en="Design trade-offs" zh="設計取捨" />

<Table
  caption={{ en: "Why these choices", zh: "為何如此選擇" }}
  columns={[
    { key: "decision", header: { en: "Decision", zh: "取捨" } },
    { key: "why", header: { en: "Why", zh: "原因" } },
  ]}
  data={[
    {
      decision: {
        en: "Serverless Next on AWS (SST + OpenNext)",
        zh: "Serverless Next 上 AWS",
      },
      why: {
        en: "Coexist with an existing private Postgres; chat needs a streaming wrapper so SSE flushes chunk-by-chunk.",
        zh: "與既有私網 Postgres 共存；chat 需要 streaming wrapper 才能逐塊 flush。",
      },
    },
    {
      decision: { en: "Cron hits own HTTP API", zh: "Cron 呼叫自家 HTTP" },
      why: {
        en: "Business logic stays in Next route handlers; the invoker Lambda only authenticates and triggers.",
        zh: "業務邏輯集中於 Next API；Invoker 只負責帶 secret 觸發。",
      },
    },
    {
      decision: {
        en: "Lazy subscription rows",
        zh: "訂閱延遲產生交易",
      },
      why: {
        en: "Avoid stuffing the ledger with future months the user never opened.",
        zh: "避免預先塞滿用戶從未開啟過的未來月份。",
      },
    },
    {
      decision: { en: "Read-only agent tools", zh: "Agent 以讀為主" },
      why: {
        en: "Summaries and Q&A without letting the model invent ledger writes.",
        zh: "可以總結與問答，但不讓模型擅自寫帳。",
      },
    },
    {
      decision: { en: "Secrets via SST Secret", zh: "密鑰使用 SST Secret" },
      why: {
        en: "Keys stay out of the repo; local uses .env and an SSH tunnel to the private DB.",
        zh: "密鑰不寫入 repo；本地使用 .env 與 SSH tunnel 連接私網 DB。",
      },
    },
  ]}
/>

<BilingualHeading level={2} en="What I use it for day to day" zh="日常如何使用" />
...
<BilingualHeading level={2} en="Takeaways" zh="小結" />
